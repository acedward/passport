// Node-side provider and wallet plumbing for the ERC20 vault fork's deploy and e2e
// tooling — copied from the project's Gate 0 spike (experiments/c2c-mint-claim-spike/
// src/node/wallet.ts), itself from the Passport cross-contract-call experiment and the
// account-custody reference harness. Three changes for this package:
//
//   * the artefact root is THIS package's managed/ (erc20-vault + the locally recompiled
//     SignetSigner), and the directory names are part of the ABI — the vault's generated
//     JS imports its callee as ../../SignetSigner/contract/index.js;
//   * every endpoint comes from an env var with a >= 10000 default, because this host runs
//     several stacks (workspace rule; question Q23);
//   * the private-state store is named for this package.
//
...
> = {
  local: {
    ...
    indexer:
      ...
      'http://localhost:18088/api/v4/graphql',
    indexerWS:
      process.env.INDEXER_WS_URL ??
      process.env.MIDNIGHT_INDEXER_WS_URL ??
      'ws://localhost:18088/api/v4/graphql/ws',
    node: process.env.NODE_URL ?? process.env.MIDNIGHT_NODE_URL ?? 'http://localhost:19944',
    proofServer:
...
export const managedPath = path.resolve(__dirname, '..', 'managed');
// `SignetSigner` is OUR 0.34.0 recompile of the vendored singleton source, NOT Sig
// Network's `ln -s node_modules/@sig-net/midnight-contract/dist/managed` (question Q20:
