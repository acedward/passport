...
/**
 * Connect to an already-deployed contract WITH witnesses (Gate 0's roots carry
 * the MIP-0012 section 6.5 `held_coin` coin store). The private state lives
 * under `privateStateId`, which defaults to the contract name so a later probe
 * reads the coin store an earlier probe wrote.
 */
export async function connectWithWitnesses(
  walletCtx: WalletContext,
  opts: DeployOptions & { witnesses: any; address?: string; privateStateId?: string; initialPrivateState?: any },
): Promise<ContractHandle> {
  const address = opts.address ?? loadDeployment(opts.name);
  if (!address) {
    throw new Error(`no deployment recorded for '${opts.name}' — run the deploy probe first`);
  ...
  const found = await (findDeployedContract as any)(providers, {
    contractAddress: address,
    compiledContract: compiledWithWitnesses(opts.name, opts.module, opts.zkPath, opts.witnesses),
    privateStateId: opts.privateStateId ?? opts.name,
    initialPrivateState: opts.initialPrivateState ?? {},
  });
  return makeHandle(opts.name, opts.module, providers, found, address);
}
