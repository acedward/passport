// Node-side provider and wallet plumbing — copied from
// experiments/cross-contract-calls/src/node/wallet.ts (itself copied from the
// account-custody reference harness), with two changes for the Gate 0 spike:
// the artefact paths name this spike's four contracts plus the linked Signet
// singleton, and the default endpoint ports are this stack's (>= 10000) rather
// than the standard ones, because this host runs other stacks.
//
...
> = {
  local: {
    ...
    indexer: process.env.INDEXER_URL ?? 'http://localhost:18088/api/v4/graphql',
    indexerWS: process.env.INDEXER_WS_URL ?? 'ws://localhost:18088/api/v4/graphql/ws',
    node: process.env.NODE_URL ?? 'http://localhost:19944',
    proofServer: process.env.PROOF_SERVER_URL ?? 'http://127.0.0.1:16300',
  },
};
...
// `SignetSigner` is a SYMLINK into node_modules/@sig-net/midnight-contract/
// dist/managed (Sig Network's own recipe, copied from the ERC20 vault's
// package.json): the declared contract type name IS the directory name, both
// for the compiler and for the generated JS the callee is imported through.
export const midZkConfigPath = path.join(managedPath, 'Mid');
export const rootZkConfigPath = path.join(managedPath, 'Root');
export const mintZkConfigPath = path.join(managedPath, 'Mint');
export const root2ZkConfigPath = path.join(managedPath, 'Root2');
...
export async function createProviders(walletCtx: WalletContext, contractZkPath: string) {
  ...
  return {
    privateStateProvider: levelPrivateStateProvider({
      ...
      privateStateStoreName: 'gate0-c2c-spike',
      privateStoragePasswordProvider: () => 'Gate0!c2c-mint-claim-spike',
