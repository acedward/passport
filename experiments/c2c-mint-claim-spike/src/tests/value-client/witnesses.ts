// VENDORED SLICE — the wallet-local coin store behind the root contracts'
// `held_coin` witness. Copied from
// experiments/cross-contract-calls/src/tests/value-client/witnesses.ts, with
// the witness name made a parameter (Gate 0's roots declare `held_coin`, the
// MIP-0012 section 6.5 name the Passport account itself uses).
//
...
export function makeCoinStoreWitnesses(witnessName = 'held_coin') {
  const lookup = (ctx: Ctx, color: Uint8Array): [CoinStorePrivateState, QualifiedCoin] => {
    const key = bytesToHex(color);
    const stored = ctx.privateState.coins[key];
    if (!stored) {
      throw new Error(
        `${witnessName} witness: no coin for color ${key} in the local store — ` +
  ...
  return { [witnessName]: lookup } as Record<string, typeof lookup>;
}
