// Evidence capture — one JSON file per probe under evidence/, copied from
// experiments/cross-contract-calls/src/tests/evidence.ts.
//
// Gate 0 changes: the default stack tags name THIS stack, and every file is
// additionally written to the project evidence folder named by
// GATE0_EVIDENCE_DIR, because project 00034 requires every probe result to
// land under evidence/00034-passport-evm-account-zswap/gate0/ the moment it
// happens (sub-plan working rule 1).
...
/** The project evidence folder; set by run-gate0.sh. */
const PROJECT_EVIDENCE_DIR = process.env.GATE0_EVIDENCE_DIR;
...
export function writeEvidence(e: Omit<Evidence, 'ranAt' | 'stack'>): void {
  ...
  const full: Evidence = {
    ...
    stack: {
      ...
      compactc: process.env.COMPACTC_VERSION ?? '0.34.0 (--feature-zkir-v3)',
      compactRuntime: '0.19.0',
      compactJs: '2.5.5-rc.8',
      midnightJs: '5.0.0-beta.7',
      ledgerV9: '1.0.0-rc.3',
      signetModule: '@sig-net/midnight 0.22.0-rc.1',
      signetSingleton: '@sig-net/midnight-contract 0.22.0-rc.1 (dist/managed)',
    },
  };
  const body = JSON.stringify(full, (_k, v) => (typeof v === 'bigint' ? v.toString() : v), 2);
  const name = `${e.testId.toLowerCase()}-${e.name}.json`;
  const file = path.join(EVIDENCE_DIR, name);
  fs.writeFileSync(file, body);
  console.log(`\n■ evidence → ${path.relative(process.cwd(), file)} [${e.verdict}]`);
  if (PROJECT_EVIDENCE_DIR) {
    fs.mkdirSync(PROJECT_EVIDENCE_DIR, { recursive: true });
    const projectFile = path.join(PROJECT_EVIDENCE_DIR, name);
    fs.writeFileSync(projectFile, body);
    console.log(`■ evidence → ${projectFile} [${e.verdict}]`);
  }
}
